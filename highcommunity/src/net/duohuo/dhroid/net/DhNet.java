...
public class DhNet {
		...
		if (cachePolicy == CachePolicy.POLICY_CACHE_ONLY
			...
			if (cacheManager != null) {
				...
				if (result != null) {
					...
					try {
						DhNet.this.getTask().doInBackground(response);
						DhNet.this.getTask().doInUI(response,
								NetTask.TRANSFER_DOUI_ForCache);
						// 缓存有数据就返回
						isCacheOk = true;
						if (DhNet.this.getTask().dialog != null) {
							DhNet.this.getTask().dialog.dismiss();
						}
					} catch (Exception e) {
					}
			...
			public void run() {
				...
				if (hasNet) {
					...
					try {
						...
						DhNet.this.getTask().transfer(response,
	...
	private void onNoNet(Boolean hasUserCache) {
		...
		DhNet.this.getTask().transfer(response, NetTask.TRANSFER_DOERROR);
	}
			...
			public void run() {
				...
				mulentity.setProgressListener(new ProgressListener() {
					...
					public void transferred(long num) {
						...
						DhNet.this.getTask().transfer(response, TRANSFER_UPLOADING);
					}
				...
				try {
					...
					if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
						...
						String aString=result.split("other")[1];
						Response myresponse = new Response(aString);
						Log.e("图片上传",aString);
						myresponse.addBundle("uploading", false);
						myresponse.addBundle("proccess", 100);
						DhNet.this.getTask().transfer(myresponse,
								NetTask.TRANSFER_DOUI);
					}
				} catch (Exception e) {
	 ...
	 */
	public void setTask(NetTask task) {
		this.task = task;
	}
}
